  ...
  return (
    <>
        <div className="container" style = {{color : props.mode ==='dark'?'white':'black'}}>
            ...
            <div className="mb-3">
            <textarea className="form-control border border-dark my-2"  style = {{backgroundColor : props.mode ==='dark'?'#4b4c50':'white', color : props.mode ==='dark'?'white':'black'}} value={text} onChange = {handleOnChange} id="myText" rows="3"></textarea>
            <button disabled= {text.length === 0} className="btn btn-secondary mx-2 my-2" onClick = {handleUpClick}>Convert to uppercase</button>
        ...
        <div>
            <div className="mb-3" style = {{color : props.mode ==='dark'?'white':'black'}}>
                <label htmlFor="wordReplace" className="form-label">Your Word</label>
                <input type="text" className="form-control border border-dark" style = {{backgroundColor : props.mode ==='dark'?'#4b4c50':'white'}} value = {yourWord} onChange = {handleOriginalWord} id="wordReplace" aria-describedby="emailHelp"/>
            </div>
            <div className="mb-3" style = {{color : props.mode ==='dark'?'white':'black'}}>
                <label htmlFor="replaceWith" className="form-label">Replace With</label>
                <input type="text" className="form-control border border-dark" style = {{backgroundColor : props.mode ==='dark'?'#4b4c50':'white'}} value = {replaceWord} onChange = {handleReplaceWord} id="replaceWith"/>
            </div>
            ...
            <button disabled= {text.length === 0} className="btn btn-primary" onClick={handleReplace}> Replace </button>
